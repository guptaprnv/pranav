...
5. **Concall transcript sourcing — resolved direction, two things still need verification.** SEBI LODR **Regulation 46(2)(o)** requires listed companies to publish the call recording on their website within 24 hours (or before next trading day) and a **written transcript within 5 working days**, hosted for a minimum of 5 years; the same materials get filed to BSE/NSE Corporate Announcements under Regulation 30. Primary sourcing path: pull the filed transcript PDF from BSE/NSE corporate-announcement feeds (mature unofficial libraries exist — e.g. BseIndiaApi, NSEPython — hitting the same endpoints the exchanges' own sites use; no official public API) or company IR pages, then parse the PDF text. **AlphaStreet India** claims full coverage of Indian earnings calls plus an institutional API — the most promising buy-vs-build shortcut, worth a direct vendor conversation on price/coverage/format before committing to building our own scraper; Screener.in and Trendlyne both have concall sections too but appear bundled into their existing subscriptions rather than separately licensable. Self-built speech-to-text is a **fallback only**, for the minority of cases where a company files audio but is late or non-compliant on the transcript — not the core pipeline, which changes the earlier framing considerably. **Needs direct verification before relying on this:** the Regulation 46(2)(o) text itself, pulled from SEBI directly rather than from company compliance pages citing it (this round's research hit a network restriction on sebi.gov.in); and AlphaStreet's actual coverage, pricing, and API terms.
